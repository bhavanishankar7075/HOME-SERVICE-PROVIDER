  ...
  useEffect(() => {
    if (user?._id) {
      ...
      const handleNewNotification = (data) => {
        ...
        // Update Snackbar for immediate notification
  ...
  const fetchPopularCities = async () => {
    ...
    try {
      ...
      const cities = response
        .slice(0, 8)
        .map(place => ({ name: place.name, placeId: place.place_id }))
...
const Navbar = () => {
  ...
  const [locationError, setLocationError] = useState('');
  const [suggestions, setSuggestions] = useState([]);
  const [popularCities, setPopularCities] = useState([]); // FIX: Initialized as empty for dynamic fetching
  const locationInputRef = useRef(null);
  const autocompleteService = useRef(null);
  const placesService = useRef(null);
  const sessionTokenRef = useRef(null);
  ...
  const handleLocationSearchChange = (e) => {
    ...
    autocompleteService.current.getPlacePredictions(
      { 
        input: value, 
        // FIX: Removed 'types' to allow for detailed local searches like "Madhuravada" and "PM Palem"
        componentRestrictions: { country: 'IN' }, 
        sessionToken: sessionTokenRef.current 
      },
      (predictions, status) => {
  ...
  return (
    <nav className={`navbar ${isScrolled ? 'navbar-scrolled' : ''}`}>
      ...
      <Snackbar
        ...
        anchorOrigin={{ vertical: 'bottom', horizontal: 'left' }}
      >
        <Alert onClose={() => setNotificationMessage({ ...notificationMessage, open: false })} severity="info" sx={{ width: '100%', cursor: 'pointer' }} onClick={() => navigate(user?.role === 'customer' ? '/profile' : '/provider/dashboard')}>
          {notificationMessage.text}
        </Alert>
      </Snackbar>
    </nav>
...
import {
  Box, Modal, Button, Typography, TextField, Alert, List, ListItem, ListItemText, Snackbar, Popover, Divider,
  Avatar, Badge, CircularProgress, ListItemAvatar, ListItemButton, IconButton, Paper
} from '@mui/material';
import { styled } from '@mui/material/styles';
import io from 'socket.io-client';
import axios from 'axios';
...
const Navbar = () => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            const { data } = await axios.get(
              `https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&result_type=street_address|locality&key=${GOOGLE_MAPS_API_KEY}`
            );
            if (data.status !== 'OK') {
              setLocationError(data.error_message || 'Failed to fetch location details.');
              return;
  ...
  const navigationItems = user
    ? user.role === 'provider'
      ? [{ label: 'Dashboard', href: '/provider/dashboard', icon: UserIcon }]
      : [{ label: 'Home', href: '/home', icon: HomeIcon }, { label: 'Services', href: '/services', icon: UserIcon }, { label: 'Find Providers', href: '/providers', icon: MapPin }]
    ...
    <nav className={`navbar ${isScrolled ? 'navbar-scrolled' : ''}`}>
      ...
      <Modal open={locationPopupOpen} onClose={() => setLocationPopupOpen(false)}>
        <Box className="location-modal">
          ...
          <Button className="detect-location-btn" onClick={detectCurrentLocation}>Detect My Location</Button>
