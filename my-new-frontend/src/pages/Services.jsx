...
const Services = () => {
  ...
  const getImageUrl = (image) => {
    if (!image) return 'https://via.placeholder.com/300x200?text=Service+Image';
...
const scrollbarStyles = (
  <GlobalStyles
    styles={{
      ...
      '*::-webkit-scrollbar-thumb': { background: '#888', borderRadius: '4px' },
      '*::-webkit-scrollbar-thumb:hover': { background: '#555' },
      '.MuiDialogContent-root': { overflowY: 'hidden !important' },
    }}
  />
);
  ...
  return (
    <Box sx={{ bgcolor: '#F9FAFB', minHeight: '100vh', pt: 8 }}>
      ...
      <Box component="main" sx={{ flexGrow: 1, px: { xs: 2, sm: 4, lg: 6 }, py: 6 }}>
        ...
        <Dialog open={isScheduleModalOpen} onClose={handleCloseSchedule} maxWidth="sm" fullWidth sx={{ '& .MuiDialog-paper': { borderRadius: 4, maxHeight: '90vh' } }}>
          <DialogTitle sx={{ bgcolor: '#4F46E5', color: 'white', py: 2, px: 3, mb: 2, textAlign: 'center', fontSize: { xs: '1.25rem', sm: '1.5rem' } }}>
            Book Service: {selectedService?.name || 'Service'}
          </DialogTitle>
          <DialogContent sx={{ pt: 3, pb: 2, overflowY: 'hidden', bgcolor: '#F9FAFB' }}>
...
<FormControl component="fieldset" fullWidth>
  <FormLabel
    ...
    sx={{
      fontWeight: 'bold',
      color: '#111827', // A strong, dark color for the title
      mb: 2,
      fontSize: '1.1rem'
    }}
  >
  ...
  <RadioGroup
    value={paymentMethod}
    onChange={(e) => setPaymentMethod(e.target.value)}
    sx={{ gap: 1.5 }} // Adds space between the radio options
  >
    <FormControlLabel
      value="Stripe"
      control={<Radio sx={{ '&.Mui-checked': { color: '#4338CA' } }} />}
...
import {
  Box, Typography, Grid, Card, CardMedia, CardContent, Button, CircularProgress, Slider,
  Dialog, DialogTitle, DialogContent, DialogActions, TextField, FormControl, InputLabel, Select, MenuItem,
  Paper, Avatar, CardActions, Tooltip, Snackbar, Alert, Rating,
  Accordion, AccordionSummary, AccordionDetails, IconButton,
  Stepper, Step, StepLabel, RadioGroup, FormControlLabel, Radio,
  AlertTitle, Chip,Container,Divider
} from "@mui/material";
import {
  FilterList as FilterIcon, RestartAlt as ClearIcon, CalendarMonth as ScheduleIcon, Info as InfoIcon,
  CleaningServices as CleaningIcon, Plumbing as PlumbingIcon, ElectricalServices as ElectricalIcon, Home as HomeIcon, CheckCircle,
  Favorite as FavoriteIcon, FavoriteBorder as FavoriteBorderIcon, Visibility as VisibilityIcon,
...
const categoryIcons = {
  "Cleaning": <CleaningIcon />, "Plumbing": <PlumbingIcon />, "Electrical": <ElectricalIcon />,
  "Home Maintenance": <HomeIcon />, "default": <InfoIcon />,
};
...
const CheckoutForm = ({ onPaymentSuccess, onPaymentError }) => {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <PaymentElement />
            <Button type="submit" disabled={isProcessing || !stripe || !elements} fullWidth variant="contained" sx={{ mt: 3, py: 1.5 }}>
                {isProcessing ? <CircularProgress size={24} /> : "Pay Now"}
            </Button>
        </form>
    );
...
const Services = () => {
  ...
  const filteredAndSortedServices = useMemo(() => {
    ...
    return result;
  }, [services, debouncedSearchTerm, filters]);
  ...
  const proceedButtonText = paymentMethod === 'Stripe' ? 'Proceed to Payment' : 'Confirm COD Booking';
  const getImageUrl = (image) => (image?.startsWith('http') ? image : `${API_URL}${image || 'https://via.placeholder.com/300x200?text=No+Image'}`);

  return (
    <Box sx={{ bgcolor: 'grey.100', minHeight: '100vh', pt: 10 }}>
        <Container maxWidth="lg" sx={{ py: 4 }}>
            <Box mb={5}>
                <Typography variant="h4" component="h1" gutterBottom sx={{ fontWeight: 'bold' }}>Featured Services</Typography>
                <Grid container spacing={3}>
                    {featuredServices.map(service => (
                        <Grid item xs={12} sm={6} md={4} key={service._id}>
                            <Card sx={{ display: 'flex', flexDirection: 'column', height: '100%', borderRadius: 3, transition: 'all 0.3s', '&:hover': { transform: 'translateY(-5px)', boxShadow: 6 } }}>
                                <CardMedia component="img" height="180" image={getImageUrl(service.image)} alt={service.name} />
                                <CardContent sx={{ flexGrow: 1 }}>
                                    <Typography variant="h6" sx={{ fontWeight: 'bold' }}>{service.name}</Typography>
                                    <Typography variant="body2" color="text.secondary">{service.description.substring(0, 80)}...</Typography>
                                </CardContent>
                                <CardActions>
                                    <Button size="small" onClick={() => navigate(`/services/${service._id}`)}>Learn More</Button>
                                </CardActions>
                            </Card>
                        </Grid>
                    ))}
            ...
            <Typography variant="h4" component="h1" gutterBottom sx={{ fontWeight: 'bold' }}>All Services</Typography>
            <Accordion sx={{ mb: 3, borderRadius: 2, '&.Mui-expanded': { margin: '0 0 24px 0' }, boxShadow: 1 }}>
                <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <FilterIcon /><Typography sx={{ fontWeight: 'medium' }}>Filters & Sort</Typography>
                    </Box>
                </AccordionSummary>
                <AccordionDetails>
                    <Grid container spacing={3} alignItems="center">
                        <Grid item xs={12} md={4}><TextField fullWidth label="Search by Name" variant="outlined" size="small" value={filters.searchTerm} onChange={e => setFilters(p => ({...p, searchTerm: e.target.value}))}/></Grid>
                        <Grid item xs={12} sm={6} md={2.5}>
                            <FormControl fullWidth size="small">
                                <InputLabel>Category</InputLabel>
                                <Select value={filters.category} label="Category" onChange={e => setFilters(p => ({...p, category: e.target.value}))}>
                                    <MenuItem value="all">All Categories</MenuItem>
                                    {[...new Set(services.map(s => s.category))].map(cat => ( <MenuItem key={cat} value={cat}>{cat}</MenuItem> ))}
                                </Select>
                            </FormControl>
                        </Grid>
                        <Grid item xs={12} sm={6} md={2.5}>
                            <FormControl fullWidth size="small">
                                <InputLabel>Sort By</InputLabel>
                                <Select value={filters.sort} label="Sort By" onChange={e => setFilters(p => ({ ...p, sort: e.target.value }))}>
                                    <MenuItem value="name_asc">Name (A-Z)</MenuItem>
                                    <MenuItem value="price_asc">Price (Low to High)</MenuItem>
                                    <MenuItem value="price_desc">Price (High to Low)</MenuItem>
                                </Select>
                        ...
                        <Grid item xs={12} md={3}><Button fullWidth variant="outlined" startIcon={<ClearIcon />} onClick={() => setFilters({ category: "all", priceRange: [0, 50000], searchTerm: "", sort: "name_asc", rating: 0 })}>Clear Filters</Button></Grid>
                        <Grid item xs={12} md={6}>
                            <Typography gutterBottom>Price Range (₹{filters.priceRange[0]} - ₹{filters.priceRange[1]})</Typography>
                            <Slider value={filters.priceRange} onChange={(e, val) => setFilters(p => ({...p, priceRange: val}))} valueLabelDisplay="auto" min={0} max={50000} step={500}/>
                        </Grid>
                        <Grid item xs={12} md={6}>
                            <Typography gutterBottom>Minimum Rating</Typography>
                             <RadioGroup row value={filters.rating} onChange={(e) => setFilters(p => ({...p, rating: Number(e.target.value)}))}>
                                <FormControlLabel value={4} control={<Radio />} label="4 ★ & up" />
                                <FormControlLabel value={3} control={<Radio />} label="3 ★ & up" />
                                <FormControlLabel value={2} control={<Radio />} label="2 ★ & up" />
                             </RadioGroup>
                        </Grid>
                    </Grid>
                </AccordionDetails>
            ...
            <Grid container spacing={3}>
                {filteredAndSortedServices.map((service) => (
                    <Grid item xs={12} sm={6} lg={4} key={service._id}>
                        <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', borderRadius: 3, transition: 'box-shadow 0.3s', '&:hover': { boxShadow: 6 }, position: 'relative' }}>
                            <IconButton onClick={() => toggleFavorite(service._id)} color={favorites.has(service._id) ? "error" : "default"} sx={{ position: 'absolute', top: 8, right: 8, bgcolor: 'rgba(255, 255, 255, 0.8)' }}>
                                {favorites.has(service._id) ? <FavoriteIcon /> : <FavoriteBorderIcon />}
                            </IconButton>
                            <CardMedia component="img" height="200" image={getImageUrl(service.image)} alt={service.name}/>
                            <CardContent sx={{ flexGrow: 1 }}>
                                <Chip label={service.category} size="small" avatar={<Avatar>{categoryIcons[service.category] || categoryIcons.default}</Avatar>} sx={{ mb: 1 }}/>
                                <Typography variant="h6" component="div" sx={{ fontWeight: 'bold' }}>{service.name}</Typography>
                                <Box sx={{ display: 'flex', alignItems: 'center', my: 1 }}>
                                    <Rating name="read-only" value={service.rating || 0} readOnly precision={0.5} size="small" />
                                    <Typography variant="body2" color="text.secondary" sx={{ ml: 1 }}>({service.reviews || 0} reviews)</Typography>
                                </Box>
                                <Typography variant="body2" color="text.secondary">{service.description.substring(0, 100)}{service.description.length > 100 && '...'}</Typography>
                            </CardContent>
                            <Divider sx={{ mx: 2 }}/>
                            <CardActions sx={{ p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <Typography variant="h5" color="primary.main" sx={{ fontWeight: 'bold' }}>₹{service.price.toLocaleString('en-IN')}</Typography>
                                <Box>
                                    <Tooltip title="Quick View"><IconButton onClick={() => handleQuickView(service)}><VisibilityIcon /></IconButton></Tooltip>
                                    <Button variant="contained" size="small" startIcon={<ScheduleIcon />} onClick={() => handleOpenSchedule(service)}>Schedule</Button>
                                </Box>
                            </CardActions>
                        </Card>
                    </Grid>
        ...
        <Dialog open={isScheduleModalOpen} onClose={handleCloseSchedule} maxWidth="sm" fullWidth>
            <DialogTitle variant="h5">Book: {selectedService?.name}</DialogTitle>
            <DialogContent>
                <Stepper activeStep={activeStep} sx={{ my: 3 }}><Step><StepLabel>Details</StepLabel></Step><Step><StepLabel>Confirm</StepLabel></Step><Step><StepLabel>Payment</StepLabel></Step><Step><StepLabel>Done</StepLabel></Step></Stepper>
                {scheduleError && <Alert severity="error" sx={{ mb: 2 }}>{scheduleError}</Alert>}
                {activeStep === 0 && (<Box><TextField label="Date" type="date" value={scheduleData.date} onChange={handleDateChange} fullWidth margin="normal" InputLabelProps={{ shrink: true }} inputProps={{ min: new Date().toISOString().split("T")[0] }} /><Typography sx={{ mt: 2, mb: 1 }}>Available Slots:</Typography>{isTimeFetching ? <CircularProgress /> : <Grid container spacing={1}>{availableTimes.length > 0 ? availableTimes.map(time => (<Grid item xs={4} sm={3} key={time}><Button fullWidth variant={scheduleData.time === time ? "contained" : "outlined"} onClick={() => setScheduleData(p=>({...p, time}))}>{time}</Button></Grid>)) : <Grid item xs={12}><Typography>No slots available.</Typography></Grid>}</Grid>}</Box>)}
                {activeStep === 1 && (
                    <Box>
                        <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
                            <Typography><b>Service:</b> {selectedService?.name}</Typography>
                            <Typography><b>Price:</b> ₹{selectedService?.price.toLocaleString('en-IN')}</Typography>
                            <Typography><b>On:</b> {new Date(scheduleData.date).toLocaleDateString()} at {scheduleData.time}</Typography>
                        </Paper>
                        <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
                            <Typography sx={{ fontWeight: 'medium', mb: 1 }}>Your Details:</Typography>
                            <Typography><b>Name:</b> {user?.name}</Typography>
                            <Typography><b>Email:</b> {user?.email}</Typography>
                            {user?.phone ? (<Typography><b>Phone:</b> {user.phone}</Typography>) : (
                                <Alert severity="warning" action={<Button color="inherit" size="small" onClick={() => navigate('/profile')}>UPDATE PROFILE</Button>} sx={{ mt: 1 }}>
                                    <AlertTitle>Phone Number Required</AlertTitle>
                                    Please add a phone number to your profile to continue.
                                </Alert>
                            )}
                        </Paper>
                        <TextField label="Service Location" value={scheduleData.location} onChange={(e) => setScheduleData(p => ({...p, location: e.target.value}))} fullWidth margin="normal" />
                        <FormControl component="fieldset" sx={{ mt: 2 }}>
                            <RadioGroup row value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)}>
                                <FormControlLabel value="Stripe" control={<Radio />} label="Pay with Card" />
                                <FormControlLabel value="COD" control={<Radio />} label="Cash on Delivery" />
                            </RadioGroup>
                        </FormControl>
                    </Box>
                )}
                {activeStep === 2 && (<Box><Typography variant="h6" gutterBottom>Complete Payment</Typography>{clientSecret && stripePromise ? (<Elements stripe={stripePromise} options={{ clientSecret }}><CheckoutForm onPaymentSuccess={() => setActiveStep(3)} onPaymentError={setScheduleError}/></Elements>) : <CircularProgress />}</Box>)}
                {activeStep === 3 && (<Box sx={{ textAlign: 'center', py: 4 }}><CheckCircle color="success" sx={{ fontSize: 60 }} /><Typography variant="h5" sx={{ mt: 2 }}>Booking Confirmed!</Typography><Typography>Your booking is successful. You will be notified shortly.</Typography></Box>)}
            </DialogContent>
            <DialogActions sx={{ p: 2 }}>
                <Button onClick={handleCloseSchedule}>Cancel</Button>
                {(activeStep === 1 || activeStep === 2) && <Button onClick={() => setActiveStep(p=>p-1)}>Back</Button>}
                {activeStep === 0 && <Button variant="contained" onClick={() => setActiveStep(1)} disabled={!scheduleData.time}>Next</Button>}
                {activeStep === 1 && <Button variant="contained" onClick={handleCreateBookingAndProceed} disabled={scheduleLoading || !user?.phone}>{scheduleLoading ? <CircularProgress size={24} /> : proceedButtonText}</Button>}
                {activeStep === 3 && <Button variant="contained" onClick={handleCloseSchedule}>Done</Button>}
            </DialogActions>
        </Dialog>
        ...
        <Dialog open={isQuickViewOpen} onClose={() => setIsQuickViewOpen(false)} maxWidth="md" fullWidth>
            <DialogTitle variant="h5">
                ...
                <IconButton onClick={() => setIsQuickViewOpen(false)} sx={{ position: 'absolute', right: 8, top: 8 }}><CloseIcon /></IconButton>
            </DialogTitle>
            <DialogContent dividers>
                <Grid container spacing={3}>
                    <Grid item xs={12} md={6}><CardMedia component="img" height="300" image={getImageUrl(selectedService?.image)} alt={selectedService?.name} sx={{ borderRadius: 2 }}/></Grid>
                    <Grid item xs={12} md={6}>
                        <Typography variant="body1" paragraph>{selectedService?.description}</Typography>
                        <Typography variant="h6" color="primary.main">₹{selectedService?.price?.toLocaleString('en-IN')}</Typography>
                        <Chip label={selectedService?.category} sx={{ my: 1 }}/>
                        <Rating name="read-only" value={selectedService?.rating || 0} readOnly precision={0.5} />
                    </Grid>
                </Grid>
            </DialogContent>
            <DialogActions>
                <Button onClick={() => { setIsQuickViewOpen(false); navigate(`/services/${selectedService?._id}`); }}>View Full Details</Button>
                <Button variant="contained" onClick={() => { setIsQuickViewOpen(false); handleOpenSchedule(selectedService); }}>Schedule Now</Button>
            </DialogActions>
        </Dialog>

        <Snackbar open={!!message?.open} autoHideDuration={6000} onClose={() => setMessage({ open: false })}>
            <Alert onClose={() => setMessage({ open: false })} severity={message?.severity || 'info'} sx={{ width: '100%' }}>{message?.text}</Alert>
        </Snackbar>
    </Box>
  );
};
...
import {
  Box, Typography, Grid, Card, CardMedia, CardContent, Button, CircularProgress, Slider,
  Dialog, DialogTitle, DialogContent, DialogActions, TextField, FormControl, InputLabel, Select, MenuItem,
  Paper, Avatar, CardActions, Tooltip, Snackbar, Alert, Rating, Toolbar,
  Accordion, AccordionSummary, AccordionDetails, IconButton,
  Stepper, Step, StepLabel, RadioGroup, FormControlLabel, Radio,
  GlobalStyles, AlertTitle
} from "@mui/material";
import {
  FilterList as FilterIcon, RestartAlt as ClearIcon, CalendarMonth as ScheduleIcon, Info as InfoIcon,
  CleaningServices as CleaningIcon, Plumbing as PlumbingIcon, ElectricalServices as ElectricalIcon, Home as HomeIcon, CheckCircle,
...
const CheckoutForm = ({ onPaymentSuccess, onPaymentError }) => {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <PaymentElement />
            <Button type="submit" disabled={isProcessing || !stripe || !elements} fullWidth variant="contained" sx={{ mt: 3 }}>
                {isProcessing ? <CircularProgress size={24} /> : "Pay Now"}
            </Button>
        </form>
    );
  ...
  return (
    <Box sx={{ bgcolor: 'grey.100', minHeight: '100vh' }}>
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
        <Toolbar />
        <Box mb={5}>
            <Typography variant="h4" component="h1" gutterBottom>Featured Services</Typography>
            <Grid container spacing={3}>
                {featuredServices.map(service => (
                    <Grid item xs={12} sm={6} md={4} key={service._id}>
                        <Card sx={{ display: 'flex', flexDirection: 'column', height: '100%', transition: 'all 0.3s', '&:hover': { transform: 'translateY(-5px)', boxShadow: 6 } }}>
                            <CardMedia component="img" height="180" image={service.image ? `${API_URL}${service.image}` : `https://via.placeholder.com/300x180?text=${service.name.charAt(0)}`} alt={service.name} />
                            <CardContent><Typography variant="h6">{service.name}</Typography></CardContent>
                        </Card>
                    </Grid>
                ))}
            </Grid>
        </Box>
        <Typography variant="h4" component="h1" gutterBottom>All Services</Typography>
        <Accordion sx={{ mb: 3, boxShadow: 2 }}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <FilterIcon /><Typography>Filters & Sort</Typography>
                </Box>
            </AccordionSummary>
            <AccordionDetails>
                <Grid container spacing={2} alignItems="center">
                    ...
                    <Grid item xs={12} md={2}><Button fullWidth variant="outlined" startIcon={<ClearIcon />} onClick={() => setFilters({ category: "all", priceRange: [0, 50000], searchTerm: "", sort: "name_asc" })}>Clear</Button></Grid>
                    <Grid item xs={12}>
                        <Typography gutterBottom>Price Range (₹{filters.priceRange[0]} - ₹{filters.priceRange[1]})</Typography>
                        <Slider value={filters.priceRange} onChange={(e, val) => setFilters(p => ({...p, priceRange: val}))} valueLabelDisplay="auto" min={0} max={50000} step={500}/>
                    </Grid>
          ...
          {filteredAndSortedServices.map((service) => (
              <Grid item xs={12} sm={6} lg={4} key={service._id}>
                <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', transition: 'box-shadow 0.3s', '&:hover': { boxShadow: 6 } }}>
                  <CardMedia component="img" height="200" image={service.image ? `${API_URL}${service.image}` : `https://via.placeholder.com/300x200?text=${service.name.charAt(0)}`} alt={service.name}/>
                  <CardContent sx={{ flexGrow: 1 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          <Typography variant="h6" component="div">{service.name}</Typography>
                          <Tooltip title={service.category}><Avatar sx={{ bgcolor: 'secondary.main' }}>{categoryIcons[service.category] || categoryIcons.default}</Avatar></Tooltip>
                      </Box>
                      <Rating name="read-only" value={service.rating || 0} readOnly precision={0.5} size="small" />
                      <Typography variant="h5" color="primary.main" sx={{ my: 1 }}>₹{service.price.toLocaleString('en-IN')}</Typography>
                      <Typography variant="body2" color="text.secondary">{service.description.substring(0, 100)}{service.description.length > 100 && '...'}</Typography>
                  </CardContent>
                  <CardActions sx={{ p: 2, justifyContent: 'space-between' }}>
                      <Box>
                          <Button size="small" startIcon={<InfoIcon />} onClick={() => navigate(`/services/${service._id}`)}>Details</Button>
                          <Button size="small" startIcon={<VisibilityIcon />} onClick={() => handleQuickView(service)} sx={{ ml: 1 }}>Quick View</Button>
                      </Box>
                      <Box>
                          <IconButton onClick={() => toggleFavorite(service._id)} color={favorites.has(service._id) ? "error" : "default"} sx={{ mr: 1 }}>
                              {favorites.has(service._id) ? <FavoriteIcon /> : <FavoriteBorderIcon />}
                          </IconButton>
                          <Button variant="contained" size="small" startIcon={<ScheduleIcon />} onClick={() => handleOpenSchedule(service)}>Schedule</Button>
                      </Box>
      ...
      <Dialog open={isScheduleModalOpen} onClose={handleCloseSchedule} maxWidth="sm" fullWidth>
        <DialogTitle variant="h5">Book Service: {selectedService?.name}</DialogTitle>
        <DialogContent>
            <Stepper activeStep={activeStep} sx={{ mb: 3 }}>
                <Step><StepLabel>Details</StepLabel></Step>
                <Step><StepLabel>Payment</StepLabel></Step>
            ...
            {activeStep === 0 && (
                <Box>
                    <TextField label="Date" type="date" value={scheduleData.date} onChange={handleDateChange} fullWidth margin="normal" InputLabelProps={{ shrink: true }} inputProps={{ min: new Date().toISOString().split("T")[0] }} />
                    <Typography variant="subtitle1" sx={{ mt: 2, mb: 1 }}>Available Slots:</Typography>
                    {isTimeFetching ? <CircularProgress /> :
                        <Grid container spacing={1}>
                            {availableTimes.length > 0 ? availableTimes.map(time => (
                                <Grid item xs={4} sm={3} key={time}><Button fullWidth variant={scheduleData.time === time ? "contained" : "outlined"} onClick={() => handleTimeSelect(time)}>{time}</Button></Grid>
                            )) : <Grid item xs={12}><Typography color="text.secondary">No available slots for this date.</Typography></Grid>}
                        </Grid>
                    }
                </Box>
            )}
            ...
            {activeStep === 1 && (
                <Box>
                    ...
                    <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
                        <Typography><b>Service:</b> {selectedService?.name}</Typography>
                        <Typography><b>Price:</b> ₹{selectedService?.price.toLocaleString('en-IN')}</Typography>
                        <Typography><b>Date:</b> {new Date(scheduleData.date).toLocaleDateString()}</Typography>
                        <Typography><b>Time:</b> {scheduleData.time}</Typography>
                    </Paper>
                    <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
                         <Typography><b>Name:</b> {user?.name}</Typography>
                         <Typography><b>Email:</b> {user?.email}</Typography>
                         ...
                         <TextField label="Service Location" value={scheduleData.location} onChange={(e) => setScheduleData({ ...scheduleData, location: e.target.value })} fullWidth margin="normal" />
                    </Paper>
            ...
            {activeStep === 2 && (
                <Box>
                    ...
                    <Typography color="text.secondary" sx={{ mb: 2 }}>Please enter your payment details below to confirm the booking.</Typography>
                    {clientSecret && stripePromise && (
                        <Elements stripe={stripePromise} options={{ clientSecret }}>
                            <CheckoutForm 
                                onPaymentSuccess={() => setActiveStep(3)} 
            ...
            {activeStep === 3 && (
                <Box sx={{ textAlign: 'center', py: 4 }}>
                    <CheckCircle color="success" sx={{ fontSize: 60 }} />
                    <Typography variant="h5" sx={{ mt: 2 }}>Booking Confirmed!</Typography>
                    <Typography color="text.secondary">Your booking is successful and is awaiting provider assignment. You will be notified shortly.</Typography>
                </Box>
            )}
        ...
        <DialogActions sx={{ p: 2 }}>
            <Button onClick={handleCloseSchedule}>Cancel</Button>
            {(activeStep === 1 || activeStep === 2) && <Button onClick={handleScheduleBack}>Back</Button>}
      ...
      <Dialog open={isQuickViewOpen} onClose={() => setIsQuickViewOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle variant="h5">{selectedService?.name}</DialogTitle>
        <DialogContent>
            <Grid container spacing={3}>
                <Grid item xs={12} md={6}><CardMedia component="img" height="300" image={selectedService?.image ? `${API_URL}${selectedService.image}` : `https://via.placeholder.com/300x300?text=${selectedService?.name?.charAt(0)}`} alt={selectedService?.name} /></Grid>
                <Grid item xs={12} md={6}>
                    <Typography variant="body1" paragraph>{selectedService?.description}</Typography>
                    <Typography variant="h6" color="primary.main">₹{selectedService?.price?.toLocaleString('en-IN')}</Typography>
                    <Typography variant="subtitle1">Category: {selectedService?.category}</Typography>
                    <Rating name="read-only" value={selectedService?.rating || 0} readOnly precision={0.5} />
                </Grid>
            </Grid>
        </DialogContent>
        <DialogActions>
            <Button onClick={() => setIsQuickViewOpen(false)}>Close</Button>
            <Button variant="contained" onClick={() => { setIsQuickViewOpen(false); navigate(`/services/${selectedService?._id}`); }}>View Full Details</Button>
        </DialogActions>
      </Dialog>
