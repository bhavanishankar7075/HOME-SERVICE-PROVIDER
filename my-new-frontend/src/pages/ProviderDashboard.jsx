...
function ProviderDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setEditData({
        ...
        location: {
          fullAddress: profileRes.data.profile?.location?.fullAddress || location || '',
          details: {
      ...
      dispatch(setLocation(profileRes.data.profile?.location?.fullAddress || ''));
  ...
  }, [token, location, dispatch, notifications]);
  ...
  useEffect(() => {
    ...
    socket.on('userUpdated', (updatedUser) => {
      console.log('Received userUpdated:', updatedUser);
      ...
      setEditData(prev => ({
        ...
        location: isDetectingLocation ? prev.location : {
          fullAddress: updatedUser.profile?.location?.fullAddress || '',
          details: {
  ...
  }, [user, token, navigate, fetchData, dispatch, isDetectingLocation]);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      setEditData(prev => ({
        ...
        location: {
          fullAddress: res.data.profile?.location?.fullAddress || '',
          details: {
  ...
  const detectLocation = async () => {
    if (!navigator.geolocation) {
      setMessage({ open: true, text: 'Geolocation is not supported by your browser.', severity: 'error' });
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      const response = await axios.get(
        `https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&result_type=street_address|locality&key=${import.meta.env.VITE_GOOGLE_MAPS_API_KEY}`
      );
      if (response.data.status === 'OK' && response.data.results.length > 0) {
        ...
        console.log('Detected location:', newLocation);
        setEditData(prev => ({ ...prev, location: newLocation }));
        dispatch(setLocation(newLocation.fullAddress));
        setMessage({ open: true, text: 'Location detected successfully!', severity: 'success' });
      } else {
        setMessage({ open: true, text: 'Failed to fetch location details.', severity: 'error' });
      }
    } catch (err) {
      console.error('Location detection error:', err.message);
      setMessage({ open: true, text: `Failed to fetch location: ${err.message}`, severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <Box sx={{ maxWidth: '1200px', margin: 'auto', p: { xs: 2, md: 3 }, mt: 4 }}>
              ...
              <Grid item xs={12} md={4} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                {profile.profile?.image ? (
                  <Avatar 
                    ...
                    onError={(e) => {
                      console.error('Failed to load provider image:', `${API_URL}${profile.profile.image}`);
        ...
        <Box>
          ...
          {filteredWorkHistory.length > 0 ? (
            filteredWorkHistory.map((work) => {
              const customerImageUrl = work.customer?.profile?.image ? `${API_URL}${encodeURI(work.customer.profile.image)}` : null;
      ...
      <Dialog open={editOpen} onClose={() => setEditOpen(false)}>
        ...
        <DialogContent>
          ...
          />
          <Button variant="outlined" onClick={detectLocation} sx={{ mt: 2, mr: 1 }}>Detect Location</Button>
      ...
      <Grid container>
        ...
        <Grid item xs={12} md={8}>
          ...
          <CardActions sx={{ justifyContent: 'flex-end', p: 2, gap: 1, flexWrap: 'wrap' }}>
            {booking.status === 'assigned' && (
              <>
                <Button variant="contained" color="success" size="small" startIcon={<CheckIcon />} onClick={() => onAction(booking._id, 'accept')}>Accept</Button>
                <Button variant="contained" color="error" size="small" startIcon={<CloseIcon />} onClick={() => onAction(booking._id, 'reject')}>Reject</Button>
              </>
            )}
            {booking.status === 'in-progress' && (
              <Button variant="contained" color="primary" size="small" startIcon={<CheckCircleIcon />} onClick={() => onAction(booking._id, 'complete')}>Mark as Complete</Button>
            )}
          </CardActions>
        </Grid>
      </Grid>
...
function ProviderDashboard() {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      setEditOpen(false);
      setMessage({ open: true, text: 'Profile updated successfully!', severity: 'success' });
      setIsDetectingLocation(false); // Reset detection flag after save
    } catch (err) {
      console.error('Profile update error:', err.response?.data || err.message);
      setMessage({ open: true, text: `Failed to update profile: ${err.response?.data?.message || err.message}`, severity: 'error'});
    } finally {
  ...
  const detectLocation = async () => {
    ...
    setLoading(true);
    setIsDetectingLocation(true); // Set flag to prevent userUpdated from overwriting
    try {
      const position = await new Promise((resolve, reject) =>
        navigator.geolocation.getCurrentPosition(resolve, reject, { enableHighAccuracy: true })
      );
  ...
  const assignedBookings = bookings.filter(b => b.status === 'assigned');
  const inProgressBookings = bookings.filter(b => b.status === 'in-progress');
  const workHistory = previousWorks;
    ...
    <Box sx={{ maxWidth: '1200px', margin: 'auto', p: { xs: 2, md: 3 }, mt: 4 }}>
      ...
      {tabValue === 1 && (
        <Box>
          <Typography variant="h5" sx={{ mb: 2 }}>New Assigned Bookings</Typography>
          {assignedBookings.length > 0 ? (
            assignedBookings.map(booking => <ProviderBookingCard key={booking._id} booking={booking} onAction={handleBookingAction} />)
          ) : (<Paper sx={{p: 3, textAlign: 'center'}}><Typography>You have no new bookings assigned.</Typography></Paper>)}
        </Box>
      )}
      ...
      {tabValue === 2 && (
        <Box>
          <Typography variant="h5" sx={{ mb: 2 }}>In-Progress Jobs</Typography>
          {inProgressBookings.length > 0 ? (
            inProgressBookings.map(booking => <ProviderBookingCard key={booking._id} booking={booking} onAction={handleBookingAction} />)
          ) : (<Paper sx={{p: 3, textAlign: 'center'}}><Typography>You have no jobs currently in progress.</Typography></Paper>)}
        </Box>
      )}
      ...
      {tabValue === 3 && (
        <Box>
          <Typography variant="h5" sx={{ mb: 2 }}>Work History</Typography>
          ...
          ) : (<Paper sx={{p: 3, textAlign: 'center'}}><Typography>You have no past jobs.</Typography></Paper>)}
        </Box>
      )}
...
function ProviderDashboard() {
  ...
  }, [user, token, navigate, fetchData, dispatch]);
...
import {
  Box, Grid, Card, CardContent, Typography, Button, CircularProgress, Alert,
  Tabs, Tab, TextField, Dialog, DialogActions, DialogContent, DialogTitle, Switch, Snackbar,
  Avatar, CardMedia, Divider, CardActions, Paper, List, ListItem, ListItemIcon, ListItemText, Badge
} from '@mui/material';
import { 
  CheckCircle as CheckCircleIcon, 
  Cancel as CancelOutlined, 
    ...
    <Box sx={{ maxWidth: '1200px', margin: 'auto', p: { xs: 2, md: 3 }, mt: 4 }}>
      ...
      {tabValue === 3 && (
        <Box>
          ...
          {workHistory.length > 0 ? (
            workHistory.map((work) => (
              <Card key={work._id} variant="outlined" sx={{mb: 2}}>
                <CardContent>
                  <Typography variant="h6">{work.service?.name}</Typography>
                  <Typography>Customer: {work.customer?.name}</Typography>
                  <Typography>Status: {work.status}</Typography>
                </CardContent>
              </Card>
            ))
          ) : (<Paper sx={{p: 3, textAlign: 'center'}}><Typography>You have no past jobs.</Typography></Paper>)}
...
import {
  Box, Grid, Card, CardContent, Typography, Button, CircularProgress, Alert,
  Tabs, Tab, TextField, Dialog, DialogActions, DialogContent, DialogTitle, Switch, Snackbar,
  Avatar, CardMedia, Divider, Chip, CardActions, AlertTitle,Paper
} from '@mui/material';
import { 
    CheckCircle as CheckCircleIcon, 
    Cancel as CancelOutlined, 
...
import io from 'socket.io-client';
import { setUser, clearUser, setLocation } from '../redux/authSlice';
import { useNavigate } from 'react-router-dom';

const API_URL = import.meta.env.VITE_API_URL || "http://localhost:5000";
const socket = io(API_URL);

const ProviderBookingCard = ({ booking, onAccept, onReject, onComplete }) => {
    return (
        <Card variant="outlined" sx={{ mb: 2, boxShadow: 2 }}>
            <Grid container>
                <Grid item xs={12} md={4}>
                ...
                <Grid item xs={12} md={8}>
                    <CardContent>
                        ...
                        <Divider sx={{ my: 1 }} />
                        <Typography variant="subtitle1" gutterBottom>Customer Details</Typography>
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}><PersonPinCircleIcon fontSize="small" sx={{ mr: 1, color: 'text.secondary' }} /><Typography variant="body2"><strong>Name:</strong> {booking.customerDetails.name}</Typography></Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}><PhoneIcon fontSize="small" sx={{ mr: 1, color: 'text.secondary' }} /><Typography variant="body2"><strong>Phone:</strong> {booking.customerDetails.phone}</Typography></Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}><LocationOnIcon fontSize="small" sx={{ mr: 1, color: 'text.secondary' }} /><Typography variant="body2"><strong>Location:</strong> {booking.location}</Typography></Box>
                        <Divider sx={{ my: 1 }} />
                        <Typography variant="subtitle1" gutterBottom>Schedule</Typography>
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}><EventIcon fontSize="small" sx={{ mr: 1, color: 'text.secondary' }} /><Typography variant="body2">{new Date(booking.scheduledTime).toLocaleDateString()}</Typography></Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}><ScheduleIcon fontSize="small" sx={{ mr: 1, color: 'text.secondary' }} /><Typography variant="body2">{new Date(booking.scheduledTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Typography></Box>
                    </CardContent>
                    <CardActions sx={{ justifyContent: 'flex-end', p: 2, gap: 1, flexWrap: 'wrap' }}>
                        {booking.status === 'assigned' && (
                            <>
                                <Button variant="contained" color="success" size="small" startIcon={<CheckIcon />} onClick={() => onAccept(booking._id)}>Accept</Button>
                                <Button variant="contained" color="error" size="small" startIcon={<CloseIcon />} onClick={() => onReject(booking._id)}>Reject</Button>
                            </>
                        )}
                        {booking.status === 'in-progress' && (
                            <Button variant="contained" color="primary" size="small" startIcon={<CheckCircleIcon />} onClick={() => onComplete(booking._id)}>Mark as Complete</Button>
                        )}
                    </CardActions>
                </Grid>
            </Grid>
...
function ProviderDashboard() {
  const { user, token, location } = useSelector((state) => state.auth);
  const dispatch = useDispatch();
  const navigate = useNavigate();
  ...
  const [editData, setEditData] = useState({ name: '', phone: '', skills: '', availability: '', location: { fullAddress: '' }, image: null });
  const [passwordData, setPasswordData] = useState({ currentPassword: '', newPassword: '', confirmPassword: '' });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setEditData({
        ...
        skills: profileRes.data.profile?.skills?.join(', ') || '',
        availability: profileRes.data.profile?.availability || '',
        location: profileRes.data.profile?.location || { fullAddress: location || '' },
        image: null,
      });
    } catch (err) {
      setMessage({ open: true, text: `Failed to fetch data: ${err.response?.data?.message || err.message}`, severity: 'error' });
    } finally {
      setLoading(false);
    }
  }, [token, location]);
  ...
  }, [user, token, dispatch, navigate, fetchData]);
  ...
  const handleBookingAction = async (bookingId, action) => {
    ...
    try {
        ...
        setMessage({ open: true, text: `Booking successfully updated!`, severity: 'success' });
        fetchData();
    } catch (err) {
        setMessage({ open: true, text: `Failed to update booking: ${err.response?.data?.message || err.message}`, severity: 'error' });
    } finally {
        setLoading(false);
    }
  };
  ...
  const handleToggleAvailability = async () => {
    ...
    try {
      ...
      setProfile(prev => prev ? { ...prev, profile: { ...prev.profile, availability: response.data.availability } } : prev);
      setEditData(prev => ({ ...prev, availability: response.data.availability }));
      setMessage({ open: true, text: `Availability updated to ${response.data.availability}!`, severity: 'success' });
    } catch (error) {
      setMessage({ open: true, text: `Failed to toggle availability: ${error.response?.data?.message || error.message}`, severity: 'error'});
    } finally {
      setLoading(false);
    }
  };
  ...
  const detectLocation = async () => {
    ...
    try {
        ...
        if (response.data.status === 'OK' && response.data.results.length > 0) {
            ...
            const newLocation = { fullAddress: formattedAddress, details: response.data.results[0].address_components };
            setEditData(prev => ({ ...prev, location: newLocation }));
  ...
  const assignedBookings = bookings.filter(b => b.status === 'assigned');
  const inProgressBookings = bookings.filter(b => b.status === 'in-progress');
  const workHistory = bookings.filter(b => ['completed', 'cancelled', 'rejected'].includes(b.status));
  ...
  return (
    <Box sx={{ maxWidth: '1200px', margin: 'auto', p: { xs: 2, md: 3 }, mt: 4 }}>
        ...
        <Tabs value={tabValue} onChange={handleTabChange} centered variant="scrollable" scrollButtons="auto" sx={{ mb: 4, backgroundColor: 'white', borderRadius: 2, boxShadow: 1 }}>
            <Tab label="Profile" />
            <Tab label={`New Bookings (${assignedBookings.length})`} />
            <Tab label={`Active Jobs (${inProgressBookings.length})`} />
            <Tab label="Work History" />
            <Tab label="Settings" />
        </Tabs>
        ...
        {tabValue === 0 && (
            <Card sx={{ p: 3 }}>
                <CardContent>
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12} md={4} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                            {profile.profile?.image ? ( <Avatar src={`${API_URL}${profile.profile.image}`} sx={{ width: 150, height: 150 }} /> ) : ( <Avatar sx={{ width: 150, height: 150, fontSize: '4rem' }}>{profile.name.charAt(0)}</Avatar> )}
                        </Grid>
                        <Grid item xs={12} md={8}>
                            <Typography sx={{ mb: 1 }}><strong>Name:</strong> {profile.name}</Typography>
                            <Typography sx={{ mb: 1 }}><strong>Email:</strong> {profile.email}</Typography>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', my: 1 }}>
                                <Typography><strong>Availability:</strong></Typography>
                                <Switch checked={profile.profile?.availability === 'Available'} onChange={handleToggleAvailability} disabled={loading} />
                                <Typography>{profile.profile?.availability || 'N/A'}</Typography>
                            </Box>
                        </Grid>
                    </Grid>
        ...
        {tabValue === 1 && (
            <Box>
                <Typography variant="h5" sx={{ mb: 2 }}>New Assigned Bookings</Typography>
                {assignedBookings.length > 0 ? (
                    assignedBookings.map(booking => <ProviderBookingCard key={booking._id} booking={booking} onAccept={() => handleBookingAction(booking._id, 'accept')} onReject={() => handleBookingAction(booking._id, 'reject')} />)
                ) : (<Paper sx={{p: 3, textAlign: 'center'}}><Typography>You have no new bookings assigned.</Typography></Paper>)}
            </Box>
        )}
        ...
        {tabValue === 2 && (
            <Box>
                <Typography variant="h5" sx={{ mb: 2 }}>In-Progress Jobs</Typography>
                {inProgressBookings.length > 0 ? (
                    inProgressBookings.map(booking => <ProviderBookingCard key={booking._id} booking={booking} onComplete={() => handleBookingAction(booking._id, 'complete')} />)
                ) : (<Paper sx={{p: 3, textAlign: 'center'}}><Typography>You have no jobs currently in progress.</Typography></Paper>)}
            </Box>
        )}
        ...
        {tabValue === 3 && (
             <Box>
                <Typography variant="h5" sx={{ mb: 2 }}>Work History</Typography>
                {workHistory.length > 0 ? (
                    workHistory.map(booking => <ProviderBookingCard key={booking._id} booking={booking} />)
                ) : (<Paper sx={{p: 3, textAlign: 'center'}}><Typography>You have no past jobs.</Typography></Paper>)}
            </Box>
        )}
        ...
        <Dialog open={editOpen} onClose={() => setEditOpen(false)}>
            ...
            <DialogContent>
                ...
                <TextField margin="dense" label="Skills (comma-separated)" type="text" fullWidth variant="standard" value={editData.skills} onChange={(e) => setEditData(prev => ({ ...prev, skills: e.target.value }))} />
                <TextField margin="dense" label="Availability" type="text" fullWidth variant="standard" value={editData.availability} onChange={(e) => setEditData(prev => ({ ...prev, availability: e.target.value }))} />
                <Button variant="outlined" onClick={detectLocation} sx={{mt: 2, mr: 1}}>Detect Location</Button>
                <Button variant="contained" component="label" sx={{ mt: 2 }}> Upload Profile Image <input type="file" hidden accept="image/*" onChange={(e) => setEditData(prev => ({ ...prev, image: e.target.files[0] }))} /> </Button>
            </DialogContent>
            <DialogActions>
                <Button onClick={() => setEditOpen(false)}>Cancel</Button>
