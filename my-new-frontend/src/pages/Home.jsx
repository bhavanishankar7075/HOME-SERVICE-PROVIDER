...
const HowItWorksSection = () => {
  const steps = [
    ...
    { icon: <PinDropIcon sx={{ fontSize: 40 }} />, title: "2. Confirm Your Details" },
    { icon: <DoneAllIcon sx={{ fontSize: 40 }} />, title: "3. Relax as We Handle It" },
  ];
  return (
            ...
            <Grid item xs={12} md={4} key={feedback._id}>
              <Paper elevation={6} sx={{ p: 4, borderRadius: 4, height: '100%', display: 'flex', flexDirection: 'column' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                  <Avatar
                    src={getImageUrl(feedback.bookingId?.customer?.profile?.image || '/images/default-user.png')}
...
const Home = () => {
  ...
  const fetchFeedbacks = useCallback(async () => {
    if (!token) {
      setErrorMessage('Please log in to view customer feedback.');
      navigate('/login');
      return;
    }
    setFeedbackLoading(true);
    try {
      console.log('Fetching feedbacks with token:', token); // Debug token
      const response = await axios.get(`${API_URL}/api/feedback`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      console.log('Feedbacks fetched:', response.data); // Debug response
      setFeedbacks(response.data || []);
      setErrorMessage(null);
    } catch (error) {
      console.error('Error fetching feedbacks:', error.response?.data?.message || error.message);
