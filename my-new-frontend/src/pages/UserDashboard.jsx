...
import { setUser, clearUser, clearNotifications } from '../redux/authSlice';
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setEditData({
        ...
        location: profileRes.data.profile?.location || { fullAddress: '' },
        image: null,
      });
  ...
  const detectLocation = async () => {
    ...
    try {
      ...
      if (response.data.status === 'OK' && response.data.results.length > 0) {
        ...
        const formData = new FormData();
        formData.append('name', updatedEditData.name.trim());
        formData.append('phone', updatedEditData.phone || '');
        formData.append('location', JSON.stringify(updatedEditData.location));
        if (updatedEditData.image) {
          formData.append('profileImage', updatedEditData.image);
        }
        const config = { headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'multipart/form-data' } };
        const { data } = await axios.put(`${API_URL}/api/users/profile`, formData, config);
        setProfile(data);
        dispatch(setUser({ user: data, token }));
        setMessage({ open: true, text: 'Location updated and saved!', severity: 'success' });
      } else {
        setMessage({ open: true, text: response.data.error_message || 'Failed to fetch location details.', severity: 'error' });
      }
    } catch (err) {
        ...
        <Card sx={{ p: { xs: 2, md: 3 } }}>
          <CardContent>
            ...
            <Box sx={{ mt: 3, display: 'flex', gap: 2, flexWrap: 'wrap' }}>
              ...
              <Button variant="contained" onClick={detectLocation} disabled={loading}>Detect My Location</Button>
            </Box>
          </CardContent>
        </Card>
      )}
...
import {
  Box, Card, CardContent, Typography, Button, CircularProgress, Alert, DialogTitle,
  Dialog, DialogActions, DialogContent, TextField, Tab, Tabs, Grid, Chip, Avatar,
  CardMedia, Divider, Rating, Paper, CardActions, Badge, List, ListItem, ListItemIcon, ListItemText, GlobalStyles, Snackbar
} from '@mui/material';
import { 
  CalendarToday as CalendarTodayIcon, 
  AccessTime as AccessTimeIcon, 
...
function UserDashboard() {
  ...
  const activeBookings = allBookings.filter(b => ['pending', 'assigned', 'in-progress'].includes(b.status));
  const previousBookings = allBookings.filter(b => ['completed', 'cancelled', 'rejected'].includes(b.status));
  ...
  return (
    <Box sx={{ maxWidth: '1200px', margin: 'auto', p: { xs: 2, md: 3 }, mt: 4 }}>
      ...
      {tabValue === 2 && (
        <Box>
          <Typography variant="h5" sx={{ mb: 2 }}>Your Booking History</Typography>
          {previousBookings.length > 0 ? (
            previousBookings.map(booking => <BookingCard key={booking._id} booking={booking} onFeedback={handleOpenFeedbackDialog} onRebook={handleRebook}/>)
          ) : (
            <Paper sx={{p: 3, textAlign: 'center'}}><Typography>You have no previous bookings.</Typography></Paper>
          )}
        </Box>
      )}
...
function UserDashboard() {
  ...
  useEffect(() => {
    ...
    const handleBookingUpdate = () => fetchData();
    const handleServiceCompleted = (data) => {
      setMessage({ open: true, text: `Service ${data.serviceName} has been completed!`, severity: 'success' });
      fetchData();
    };
  ...
  const handleTabChange = (event, newValue) => {
    setTabValue(newValue);
    if (newValue !== 4) { // Clear notifications when leaving the Notifications tab
      dispatch(clearNotifications());
      console.log('Notifications cleared on tab change to:', newValue);
    }
  };
  ...
  const handleConfirmFeedback = async () => {
    ...
    try {
      await axios.post(`${API_URL}/api/feedback`, 
        {
          bookingId: bookingForAction._id,
          serviceId: bookingForAction.service._id,
          rating: feedbackData.rating,
          comment: feedbackData.comment,
        },
        { headers: { Authorization: `Bearer ${token}` } }
    ...
    <Box sx={{ maxWidth: '1200px', margin: 'auto', p: { xs: 2, md: 3 }, mt: 4 }}>
      ...
      <Dialog open={feedbackOpen} onClose={() => setFeedbackOpen(false)}>
        ...
        <DialogContent>
          <Typography component="legend">Your Rating</Typography>
          <Rating name="booking-rating" value={feedbackData.rating} onChange={(event, newValue) => { setFeedbackData(prev => ({ ...prev, rating: newValue })); }} />
          <TextField autoFocus margin="dense" label="Your Comments" type="text" fullWidth variant="outlined" multiline rows={4} value={feedbackData.comment} onChange={(e) => setFeedbackData(prev => ({ ...prev, comment: e.target.value }))} />
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setFeedbackOpen(false)}>Cancel</Button>
          <Button onClick={handleConfirmFeedback} variant="contained">Submit Feedback</Button>
        </DialogActions>
      </Dialog>
...
import {
  Box, Card, CardContent, Typography, Button, CircularProgress, Alert, DialogTitle,
  Dialog, DialogActions, DialogContent, TextField, Tab, Tabs, Grid, Chip, Avatar,
  CardMedia, Divider, Rating, AlertTitle, IconButton, Snackbar, GlobalStyles,Paper,CardActions
} from '@mui/material';
import { 
    CalendarToday as CalendarTodayIcon, 
    AccessTime as AccessTimeIcon, 
...
// A helper component to display a single booking card
const BookingCard = ({ booking, onCancel, onFeedback, onRebook }) => {
    const canCancel = ['pending', 'assigned'].includes(booking.status);
    const canGiveFeedback = booking.status === 'completed';
    const canRebook = ['completed', 'cancelled', 'rejected'].includes(booking.status);
            ...
            <Grid container>
                ...
                <Grid item xs={12} sm={8}>
                    <CardContent>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}><CalendarTodayIcon fontSize="small" sx={{ mr: 1, color: 'text.secondary' }} /><Typography variant="body2">{new Date(booking.scheduledTime).toLocaleDateString()}</Typography></Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}><AccessTimeIcon fontSize="small" sx={{ mr: 1, color: 'text.secondary' }} /><Typography variant="body2">{new Date(booking.scheduledTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Typography></Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}><SupportAgentIcon fontSize="small" sx={{ mr: 1, color: 'text.secondary' }} /><Typography variant="body2">Provider: {booking.provider?.name || 'Awaiting Assignment'}</Typography></Box>
                    </CardContent>
                    <Divider />
                    <CardActions sx={{ justifyContent: 'flex-end', p: 2, flexWrap: 'wrap', gap: 1 }}>
                        {canCancel && <Button size="small" color="error" startIcon={<CancelIcon />} onClick={() => onCancel(booking)}>Cancel Booking</Button>}
...
function UserDashboard() {
  ...
  const fetchData = useCallback(async () => {
    if (!token) {
      setMessage({ open: true, text: 'Authentication token missing.', severity: 'error' });
      setLoading(false);
      return;
    }
    setLoading(true);
  ...
  useEffect(() => {
    ...
    const handleBookingUpdate = () => fetchData();
    socket.on('bookingStatusUpdate', handleBookingUpdate);
    socket.on('newBookingAssigned', handleBookingUpdate);
    
    return () => {
      socket.off('bookingStatusUpdate', handleBookingUpdate);
      socket.off('newBookingAssigned', handleBookingUpdate);
    };
  }, [fetchData, user]);
    ...
    try {
      ...
      if (response.data.status === 'OK' && response.data.results.length > 0) {
        ...
        const updatedEditData = {
          ...
          location: {
            fullAddress: formattedAddress,
            details: response.data.results[0].geometry || {},
          },
        };
        setEditData(updatedEditData);
        const formData = new FormData();
  ...
  return (
    <Box sx={{ maxWidth: '1200px', margin: 'auto', p: { xs: 2, md: 3 }, mt: 4 }}>
      <Typography variant="h3" gutterBottom sx={{ textAlign: 'center', mb: 4 }}>Customer Dashboard</Typography>
      <Tabs value={tabValue} onChange={handleTabChange} centered variant="scrollable" scrollButtons="auto" sx={{ mb: 4 }}>
        <Tab label="Profile" />
      ...
      {tabValue === 1 && (
          <Box>
            <Typography variant="h5" sx={{ mb: 2 }}>Track Your Active Services</Typography>
            {activeBookings.length > 0 ? (
                activeBookings.map(booking => <BookingCard key={booking._id} booking={booking} onCancel={handleOpenCancelDialog} />)
            ) : (
                <Paper sx={{p: 3, textAlign: 'center'}}><Typography>You have no active bookings to track.</Typography></Paper>
            )}
          </Box>
      ...
      <Dialog open={cancelOpen} onClose={() => setCancelOpen(false)}>
        <DialogTitle>Confirm Cancellation</DialogTitle>
        <DialogContent><Typography>Are you sure you want to cancel your booking for "{bookingForAction?.service?.name}"?</Typography><Typography color="text.secondary" variant="body2">This action cannot be undone.</Typography></DialogContent>
        <DialogActions>
          <Button onClick={() => setCancelOpen(false)}>No, Keep It</Button>
          <Button onClick={handleConfirmCancel} color="error" variant="contained">Yes, Cancel</Button>
        </DialogActions>
