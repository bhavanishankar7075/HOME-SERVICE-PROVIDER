        ...
        try {
          ...
          setFeedbackError(
            error.response?.status === 401
              ? "Unauthorized: Please log in to view feedback"
...
import { ServicesContext } from '../context/ServicesContext';
import {
  Box, Typography, Button, Grid, Paper, CardMedia, Rating, Chip, List, ListItem, ListItemIcon, ListItemText, Divider, Select, MenuItem, FormControl, InputLabel, Skeleton,Container,CardActionArea
} from '@mui/material';
import { LocalizationProvider } from '@mui/x-date-pickers/LocalizationProvider';
import { AdapterDateFns } from '@mui/x-date-pickers/AdapterDateFns';
import { DatePicker } from '@mui/x-date-pickers/DatePicker';
import { CheckCircleOutline as CheckCircleIcon, Star as StarIcon } from '@mui/icons-material';
...
const ServiceDetails = () => {
  ...
  const getImageUrl = (imagePath) => {
    if (!imagePath) return "https://via.placeholder.com/600x400.png?text=Image+Not+Available";
    return imagePath.startsWith('http') ? imagePath : `${API_URL}${imagePath}`;
  };
  ...
  useEffect(() => {
    if (services.length > 0) {
      ...
      if (foundService) {
        setFeaturedImage(getImageUrl(foundService.image));
      }
    }
  }, [id, services]);
  ...
  const relatedServices = services.filter(s => s.category === service?.category && s._id !== service?._id).slice(0, 3);
            ...
            <Container maxWidth="lg">
                <Grid container spacing={4}>
                    ...
                    <Grid item xs={12} md={6}>
                        ...
                        <Skeleton variant="rectangular" width="100%" height={120} sx={{ my: 2, borderRadius: 2 }} />
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Box sx={{ pt: { xs: 10, md: 12 }, pb: 8, bgcolor: '#F9FAFB' }}>
        <Container maxWidth="lg">
          <Grid container spacing={{ xs: 4, md: 6 }}>
              ...
              <Grid container spacing={1} sx={{ mt: 1 }}>
                {allImages.map((img, index) => (
                  <Grid item xs={3} key={index}>
                    <CardActionArea onClick={() => setFeaturedImage(getImageUrl(img))}>
                      <CardMedia
                        component="img"
                        ...
                        sx={{
                          height: 80,
                          borderRadius: 2,
                          border: featuredImage === getImageUrl(img) ? '3px solid' : '3px solid transparent',
            ...
            <Grid item xs={12} md={6}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
                ...
                <Typography sx={{ ml: 1, color: 'text.secondary' }}>(32 reviews)</Typography>
              </Box>
              ...
              <Box>
                <Typography variant="h4" sx={{ fontWeight: 'bold', color: '#4F46E5', mb: 3 }}>
                  ₹{service.price.toLocaleString('en-IN')}
                </Typography>
              </Box>
              
              <Paper elevation={2} sx={{ p: 3, borderRadius: 3, bgcolor: 'white' }}>
                <Typography variant="h6" sx={{ mb: 2, fontWeight: 'medium' }}>Schedule Your Service</Typography>
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth>
                      ...
                      >
                        <MenuItem value={'09:00 - 12:00'}>Morning (9am - 12pm)</MenuItem>
                        <MenuItem value={'12:00 - 15:00'}>Afternoon (12pm - 3pm)</MenuItem>
                        <MenuItem value={'15:00 - 18:00'}>Evening (3pm - 6pm)</MenuItem>
                      </Select>
                    </FormControl>
                  </Grid>
                </Grid>
                <Button
                  ...
                  sx={{ mt: 2, py: 1.5, fontSize: '1rem', fontWeight: 'bold', bgcolor: '#4F46E5', '&:hover': { bgcolor: '#4338CA' } }}
                >
                  Book Now
                </Button>
              </Paper>
          ...
          <Grid container spacing={{ xs: 4, md: 8 }}>
            <Grid item xs={12} md={7}>
                <Typography variant="h5" sx={{ fontWeight: 'bold', mb: 3 }}>What's Included</Typography>
                <List>
                    {service.inclusions?.map((item, index) => (
            ...
            <Grid item xs={12} md={5}>
                <Typography variant="h5" sx={{ fontWeight: 'bold', mb: 3 }}>Customer Reviews</Typography>
                {[...Array(2)].map((_, i) => (
                    <Paper key={i} elevation={0} variant="outlined" sx={{ p: 2, mb: 2, borderRadius: 2 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                            <Typography sx={{ fontWeight: 'bold', mr: 1 }}>Anjali D.</Typography>
                            <Rating value={5} size="small" readOnly />
                        </Box>
                        <Typography variant="body2" color="text.secondary">"Amazing work, very professional and on time. Highly recommended!"</Typography>
                    </Paper>
                ))}
            </Grid>
          </Grid>
          ...
          {relatedServices.length > 0 && (
            <>
                <Divider sx={{ my: 6 }} />
                <Typography variant="h5" sx={{ fontWeight: 'bold', mb: 4 }}>Related Services</Typography>
                <Grid container spacing={4}>
                    {relatedServices.map(relService => (
                        <Grid item xs={12} sm={6} md={4} key={relService._id}>
                            <Card elevation={2} sx={{ height: '100%', borderRadius: 3 }}>
                                <CardActionArea onClick={() => navigate(`/services/${relService._id}`)}>
                                    <CardMedia component="img" image={getImageUrl(relService.image)} height="160" />
                                    <CardContent>
                                        <Typography variant="h6" sx={{ fontWeight: 'medium' }}>{relService.name}</Typography>
                                        <Typography variant="body2" color="text.secondary" sx={{ my: 1 }}>{relService.description.substring(0, 70)}...</Typography>
                                        <Typography variant="h6" color="primary" sx={{ fontWeight: 'bold' }}>₹{relService.price.toLocaleString('en-IN')}</Typography>
                                    </CardContent>
                                </CardActionArea>
                            </Card>
                        </Grid>
