...
const handleStripeWebhook = asyncHandler(async (req, res) => {
  ...
  // Handle Subscription Events
    ...
    if (user) {
      ...
      // Re-fetch full user for socket emit
      const updatedUser = await User.findById(userId).select('-password');
      if (global.io) {
        console.log(`[Webhook] Emitting userUpdated for user ${userId}`);
        global.io.to(userId.toString()).emit('userUpdated', updatedUser.toObject());
      }
    }
  }
  // This event handles both payment failures and scheduled cancellations
  else if (event.type === 'customer.subscription.updated') {
    const subscription = event.data.object;
    const user = await User.findOne({ stripeCustomerId: subscription.customer });
    ...
    if (user) {
      ...
      const updatedUser = await User.findById(user._id).select('-password');
      if (global.io) {
        console.log(`[Webhook] Emitting userUpdated for user ${user._id}`);
        global.io.to(user._id.toString()).emit('userUpdated', updatedUser.toObject());
      }
    }
  }
  // This event handles immediate cancellations
  else if (event.type === 'customer.subscription.deleted') {
    const subscription = event.data.object;
    const user = await User.findOne({ stripeCustomerId: subscription.customer });
...
const handleStripeWebhook = asyncHandler(async (req, res) => {
    ...
    // This event handles both payment failures and scheduled cancellations
        ...
        if (user) {
            ...
            // This checks if the user scheduled the cancellation at the end of the period
            if (subscription.cancel_at_period_end) {
                user.subscriptionStatus = 'canceled';
                user.subscriptionTier = 'free';
            }
...
const Plan = require('../models/Plan');   // <-- Added Plan model
const Joi = require('joi');
...
// 1. Create or Retrieve Stripe Payment Intent (Unchanged)
const createStripePaymentIntent = asyncHandler(async (req, res) => {
  if (!stripe) {
    res.status(500);
    throw new Error('Stripe payment service is not available');
...
// 2. Handle Cash on Delivery (COD) Orders (Unchanged)
const handleCashOnDeliveryOrder = asyncHandler(async (req, res) => {
    const { bookingId } = req.body;
    if (!bookingId) {
        res.status(400);
...
// 3. Handle Stripe Webhook Events (Updated)
const handleStripeWebhook = asyncHandler(async (req, res) => {
    const sig = req.headers['stripe-signature'];
    let event;
    ...
    // Event: A provider successfully subscribes to a new plan
    else if (event.type === 'checkout.session.completed') {
        const session = event.data.object;
        const userId = session.client_reference_id; // Our User ID
        ...
        if (user) {
            ...
            if (plan) {
                user.subscriptionTier = plan.name.toLowerCase(); // e.g. "pro"
            }
            await user.save();
    ...
    // Event: A subscription is updated (e.g., payment fails, plan changes)
    else if (event.type === 'customer.subscription.updated') {
        const subscription = event.data.object;
        const user = await User.findOne({ stripeCustomerId: subscription.customer });

        if (user) {
            user.subscriptionStatus = subscription.status; // e.g., 'active', 'past_due'
            await user.save();
            console.log(`Subscription status updated to "${subscription.status}" for user: ${user._id}`);
        }
    }

    // Event: A subscription is canceled
    else if (event.type === 'customer.subscription.deleted') {
        const subscription = event.data.object;
        const user = await User.findOne({ stripeCustomerId: subscription.customer });
        ...
        if (user) {
            user.subscriptionStatus = 'canceled';
            user.subscriptionTier = 'free'; // Downgrade the user to the free tier
            await user.save();
            console.log(`Subscription canceled for user: ${user._id}. Downgraded to free tier.`);
        }
    }
