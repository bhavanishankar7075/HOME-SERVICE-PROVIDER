...
// --- 2. ADDED THE NEW ROUTE FOR CUSTOMERS TO GET THEIR MESSAGES ---
router.get(
    '/messages',
    authMiddleware(['customer']), // Only customers can see their own messages
    getCustomerMessages
);
...
// Add self-toggle routes for providers
...
router.post('/login', loginUser);
router.get('/profile', authMiddleware(['customer', 'provider', 'admin']), getProfile);
router.put('/profile', authMiddleware(['customer', 'provider', 'admin']), updateProfile); // Corrected syntax
router.put('/change-password', authMiddleware(['customer', 'provider', 'admin']), changePassword);
router.delete('/delete', authMiddleware(['customer', 'provider']), deleteAccount);
