...
import {
  ...
  Drawer, List, ListItem, ListItemIcon, ListItemText, IconButton, Badge, Toolbar, Divider
} from '@mui/material';
import {
...
const AdminDashboard = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setMonthlyRevenue(revenueResponse.data.monthly || {});
  ...
  useEffect(() => {
    console.log('Monthly Revenue in useEffect:', monthlyRevenue);
  ...
  useEffect(() => {
    if (chartRef.current && !loading) {
      ...
      if (chartInstance) {
        chartInstance.data = barChartData;
        chartInstance.options = barChartOptions;
  ...
  const handleExportRevenue = () => {
    console.log('AdminDashboard: Exporting monthly revenue data');
    const profitMargin = 0.3; // 30% profit margin
    const csv = [
      'Month,Revenue (INR),Profit (INR)',
      ...months.map(month => {
        const revenue = monthlyRevenue[month] || 0;
        const profit = revenue * profitMargin;
        return `"${month}","${revenue.toFixed(2)}","${profit.toFixed(2)}"`;
      }),
    ].join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'monthly_revenue_export.csv';
    a.click();
  };
  ...
  const profitMargin = 0.3; // 30% profit margin
  const revenueValues = months.map(month => monthlyRevenue[month] || 0);
  const totalAnnualRevenue = revenueValues.reduce((sum, rev) => sum + rev, 0);
  const averageMonthlyRevenue = totalAnnualRevenue / (revenueValues.filter(rev => rev > 0).length || 1);
  const bestMonth = months.reduce((best, month) => (!monthlyRevenue[best] || monthlyRevenue[month] > monthlyRevenue[best]) ? month : best, null);
  const monthlyProfit = revenueValues.map(rev => rev * profitMargin);
  const totalAnnualProfit = totalAnnualRevenue * profitMargin;
  const averageMonthlyProfit = averageMonthlyRevenue * profitMargin;
  const bestProfitMonth = bestMonth;
    ...
    datasets: [
      {
        ...
        data: revenueValues,
        backgroundColor: months.map(month => month === bestMonth ? 'rgba(255, 99, 132, 0.6)' : 'rgba(74, 144, 226, 0.6)'),
      ...
      },
      {
        ...
        data: monthlyProfit,
        backgroundColor: months.map(month => month === bestProfitMonth ? 'rgba(75, 192, 192, 0.6)' : 'rgba(75, 192, 192, 0.4)'),
      ...
      },
      {
        ...
        type: 'line',
        data: months.map(() => averageMonthlyRevenue),
      ...
      },
      {
        ...
        type: 'line',
        data: months.map(() => averageMonthlyProfit),
  ...
  const subscriptionCounts = users
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: '#f4f6f8', overflow: 'hidden' }}>
      <Drawer
        variant="permanent"
        sx={{
          width: 240,
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3, width: { md: `calc(100% - 240px)` }, overflowY: 'auto' }}>
        <Toolbar />
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4, flexWrap: 'wrap', gap: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <IconButton onClick={toggleSidebar} sx={{ display: { md: 'none' }, mr: 1 }}><Menu /></IconButton>
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            ...
            <Button variant="contained" onClick={handleExportUsers} disabled={loading}>
              Export Users
            </Button>
          </Box>
        </Box>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6} md={4}>
                <Paper sx={{ p: 2, textAlign: 'center' }}>
                  <Typography variant="h6">Active Provider Subscription Plans</Typography>
              ...
              <Grid item xs={12} lg={6}>
                <Paper sx={{ p: 2, borderRadius: 2 }}>
                  <Typography variant="h6">Monthly Revenue & Profit</Typography>
                  <Grid container spacing={2} sx={{ my: 1, textAlign: 'center' }}>
                    <Grid item xs={12} sm={4}>
                      <Typography variant="body2" color="text.secondary">Total Revenue (Year)</Typography>
                      <Typography variant="h6">{new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(totalAnnualRevenue)}</Typography>
                    </Grid>
                    <Grid item xs={12} sm={4}>
                      <Typography variant="body2" color="text.secondary">Monthly Avg. Revenue</Typography>
                      <Typography variant="h6">{new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(averageMonthlyRevenue)}</Typography>
                    </Grid>
                    <Grid item xs={12} sm={4}>
                      <Typography variant="body2" color="text.secondary">Best Revenue Month</Typography>
                      <Typography variant="h6">{bestMonth || 'N/A'}</Typography>
                    </Grid>
                    <Grid item xs={12} sm={4}>
                      <Typography variant="body2" color="text.secondary">Total Profit (Year)</Typography>
                      <Typography variant="h6">{new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(totalAnnualProfit)}</Typography>
                    </Grid>
                    <Grid item xs={12} sm={4}>
                      <Typography variant="body2" color="text.secondary">Monthly Avg. Profit</Typography>
                      <Typography variant="h6">{new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(averageMonthlyProfit)}</Typography>
                    </Grid>
                    <Grid item xs={12} sm={4}>
                      <Typography variant="body2" color="text.secondary">Best Profit Month</Typography>
                      <Typography variant="h6">{bestProfitMonth || 'N/A'}</Typography>
                    </Grid>
                  </Grid>
                  <Divider sx={{ mb: 2 }} />
                  <ChartContainer>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'center', mt: 2 }}>
                    <Button variant="contained" onClick={handleExportRevenue}>Export Revenue Data</Button>
                  </Box>
                </Paper>
              </Grid>
            </Grid>
