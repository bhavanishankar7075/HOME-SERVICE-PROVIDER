...
function PlansPage() {
  ...
  const [confirmCancelSubscriptionOpen, setConfirmCancelSubscriptionOpen] = useState(false);
  const [providerToCancel, setProviderToCancel] = useState(null);
  ...
  const filteredProviders = useMemo(() => {
    return providers.filter(provider => {
      ...
      const matchesTier = filters.subscriptionTier === 'all' || 
        provider.subscriptionTier === filters.subscriptionTier ||
        (filters.subscriptionTier === 'free' && !provider.subscriptionTier);

      const matchesStatus = filters.subscriptionStatus === 'all' || 
        provider.subscriptionStatus === filters.subscriptionStatus ||
        (filters.subscriptionStatus === 'inactive' && !provider.subscriptionStatus);

      const matchesExpiry = filters.expiryWarning === 'all' ||
        (filters.expiryWarning === 'expiring' && provider.subscriptionStatusMessage?.includes('expires in')) ||
        (filters.expiryWarning === 'past_due' && provider.subscriptionStatusMessage?.includes('Payment required')) ||
        (filters.expiryWarning === 'none' && !provider.subscriptionStatusMessage);
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Toolbar sx={{ mb: 3, bgcolor: 'background.paper', boxShadow: 1, borderRadius: 1 }}>
      ...
      {loading ? (
        ...
        <>
          ...
          <Typography variant="h5" sx={{ mt: 5, mb: 2, fontWeight: 'medium' }}>
          ...
          <Paper sx={{ p: 2, mb: 3, display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <FormControl sx={{ minWidth: 150 }}>
              <InputLabel>Expiry Warning</InputLabel>
              <Select
                name="expiryWarning"
                value={filters.expiryWarning}
                onChange={handleFilterChange}
              ...
              >
                ...
                <MenuItem value="past_due">Past Due</MenuItem>
                <MenuItem value="none">No Warnings</MenuItem>
              </Select>
            </FormControl>
            <Button
              variant="outlined"
              onClick={() => setFilters({ search: '', subscriptionTier: 'all', subscriptionStatus: 'all', expiryWarning: 'all' })}
            >
              Clear Filters
            </Button>
          </Paper>
          {filteredProviders.length === 0 ? (
            <Alert severity="info">No providers match the selected filters.</Alert>
          ) : (
              ...
              <Table>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell sx={{ fontWeight: 'bold' }}>Subscription Tier</TableCell>
                    <TableCell sx={{ fontWeight: 'bold' }}>Status</TableCell>
                    <TableCell sx={{ fontWeight: 'bold' }}>Bookings (Current/Limit)</TableCell>
                    <TableCell sx={{ fontWeight: 'bold' }}>Expiry Warning</TableCell>
                    <TableCell sx={{ fontWeight: 'bold' }}>Actions</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {filteredProviders.map((provider) => (
                    <TableRow key={provider._id} sx={{ '&:hover': { bgcolor: 'grey.100' } }}>
                      ...
                      <TableCell>
                        {provider.subscriptionStatusMessage || 
                          (provider.subscriptionTier === 'free' ? 'No expiration' : 'Active, no warnings')}
                      </TableCell>
                      <TableCell>
      ...
      <Dialog open={isModalOpen} onClose={handleCloseModal} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ pt: 2 }}>
          ...
          <TextField
            ...
            onChange={handleInputChange}
            helperText="Example: Unlimited Bookings, Reduced Commission"
          />
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={confirmCancelSubscriptionOpen} onClose={handleCloseCancelSubscriptionConfirm} maxWidth="xs" fullWidth>
