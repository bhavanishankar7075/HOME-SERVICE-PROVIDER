...
// Import your axios instance and clearUser action
...
// Internal wrapper to handle token validation logic
const AppContent = () => {
  ...
  useEffect(() => {
    const validateSession = async () => {
      // If Redux thinks we are logged in, check if the token is actually valid with the server
      if (isAuthenticated && token) {
        try {
          // This call will trigger your 401 interceptor if the token is expired
          await axiosInstance.get('/api/admin/profile'); 
          console.log('App: Session validated successfully');
        } catch (error) {
          // If the server returns 401, your axios interceptor should handle dispatch(clearUser())
          // If not, we handle it here as a backup
          if (error.response && error.response.status === 401) {
            console.log('App: Session expired, clearing user state');
            dispatch(clearUser());
          }
        }
      }
